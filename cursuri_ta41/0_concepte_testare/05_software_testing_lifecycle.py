...
1. Test planning
O etapa administrativa, pregatitoare a intregului proces de testare,
care presupune planificarea activitatilor de testare si a procesului de testare:
...
Criterii de intrare (entry criteria) = conditii care trebuie indeplinite pentru a putea incepe procesul de testare
                                    (toate riscurile - sau macar cele majore - au fost mitigate, cerintele de business
                                    sunt definite, rolurile au fost alocate, planul de testare a fost scris si agreat de
                                    toata lumea)

Criterii de iesire (exit criteria) = conditii care trebuie indeplinite pentru a putea finaliza procesul de testare
                                (sa executam toate testele sau un anumit procentaj specific, am ajuns la deadline,
                                am terminat bugetul, nu am mai gasit bug-uri de o severitate mare sau foarte mare intr-un
...
a) se analizeaza cerintele de business pentru a evalua daca sunt corecte, complete si daca putem sa facem testare urmand
acele cerinte de business. Scopul acestei analize (testare statica) este de a asigura gasirea timpurie a bug-urilor
si asigurarea unui proces de testare calitativ
...
cerintele de business analizate si sparte in conditii de testare si teste, se grupeaza testele in suite de teste,
se creeaza datele de testare identificate in etapa anterioara (ex: daca la test design am identificat ca am un test de
filtrare useri, in etapa de implementation fie o sa imi creez userii pentru filtrare, fie o sa ii import dintr-un fisier
extern)

Suite de teste = o grupare de teste care au acelasi obiectiv (teste functionale, teste de regresie, teste non-functionale,
teste blackbox, teste whitebox, teste specifice unui release etc)
Suitele de teste poarta un nume diferit in functie de aplicatia pe care o folosim pentru testare (zephyr - test cycle,
testrail - test suite, testlink - test plan, practitest - test library)

6. Test execution
- se executa testele definite in etapa de analiza
- se raporteaza rezultatele testelor intr-un tool de gestiune a testarii
    (exemple de tool-uri: zephyr, alm, testrail, practitest, testlink, asana, bugzilla,mantis)
...
- se creeaza un test report = un document care contine rezultatul tuturor activitatilor desfasurate in timpul procesului
de testare (care au fost testerii implicati, care au fost cerintele de business acoperite, functionalitatile care au fost
in scope / not in scope, cate teste am executat, cate au fost passed si cate au fost failed, cate bug-uri am gasit,
...
2. Component testing = verificarea functionalitatii componentelor individuale dintr-o aplicatie, neconectate neaparat cu
                        alte componente (eX: pagina de checkout). Daca o componenta depinde de anumite functionalitati
                        importante care nu sunt implementate, atunci este posibil sa se foloseasca niste simulatori
                        (stubs / drivers)

3. Integration testing = verificarea felului in care mai multe componente sau sisteme comunica intre ele
    - Component integration testing (ex: legarea paginii de produs cu pagina de checkout)
    - System integration testing (ex: legarea site-ului de comert electronic cu paypal)
